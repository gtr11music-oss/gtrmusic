...
export function uploadToPodcast(
  item: UploadItem
): PodcastEpisode {
  return {
    id: item.id,
    title: item.title,
    showId: `show-${item.uploadedBy ?? "user"}`,
...
export function getPublicTracks(
  publishedUploads: UploadItem[]
): Track[] {
...
export function getPublicPodcasts(
  publishedUploads: UploadItem[]
): PodcastEpisode[] {
  const fromUploads = publishedUploads
